...
public class FoldWhileListMaterializer<E, F> extends SuppliedListMaterializer<F> {

  private Predicate<? super F> condition;
  private F identity;
  private BinaryFunction<? super F, ? super E, ? extends F> operation;
  private ListMaterializer<E> wrapped;

  public FoldWhileListMaterializer(final @NotNull ListMaterializer<E> wrapped, final F identity,
  ...
  public ListMaterializer<F> get() throws Exception {
    ...
    if (!wrapped.materializeEmpty()) {
      ...
      while (condition.test(current) && iterator.hasNext()) {
