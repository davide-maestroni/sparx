...
class GroupCollectionMaterializer<E, L extends List<E>> implements CollectionMaterializer<L> {

  private volatile CollectionMaterializer<L> state;

  GroupCollectionMaterializer(@NotNull final CollectionMaterializer<E> wrapper, final int maxSize,
  ...
  private class ImmaterialFillerState implements CollectionMaterializer<L> {
    ...
    private ImmaterialFillerState(@NotNull final CollectionMaterializer<E> wrapped,
        final int maxSize,
        final E filler, @NotNull final Function<? super List<E>, ? extends L> mapper) {
    ...
    public L materializeElement(final int index) {
      ...
      try {
        ...
        if (++elementsCount == (wrapped.knownSize() + maxSize - 1) / maxSize) {
          ...
          state = new ListToCollectionMaterializer<L>(elements);
        }
        return element;
  ...
  private class ImmaterialState implements CollectionMaterializer<L> {
    ...
    private ImmaterialState(@NotNull final CollectionMaterializer<E> wrapped, final int maxSize,
        @NotNull final Function<? super List<E>, ? extends L> mapper) {
      this.wrapped = wrapped;
      this.maxSize = maxSize;
      this.mapper = mapper;
