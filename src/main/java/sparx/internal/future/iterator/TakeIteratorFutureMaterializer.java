...
public class TakeIteratorFutureMaterializer<E> extends AbstractIteratorFutureMaterializer<E> {
  ...
  private static final Logger LOGGER = Logger.getLogger(
      TakeIteratorFutureMaterializer.class.getName());

  private final boolean isMaterializedAtOnce;
  private final int knownSize;

  public TakeIteratorFutureMaterializer(@NotNull final IteratorFutureMaterializer<E> wrapped,
  ...
  private class ImmaterialState implements IteratorFutureMaterializer<E> {
      ...
      if (index >= maxElements) {
        ...
        wrapped.materializeNext(new CancellableIndexedFutureConsumer<E>() {
          ...
          public void cancellableAccept(final int size, final int index, final E element)
              throws Exception {
            final int currentIndex = ImmaterialState.this.index++;
            if (currentIndex >= maxElements) {
              setDone(EmptyIteratorFutureMaterializer.<E>instance());
              consumer.complete(0);
            } else {
              consumer.accept(safeSize(size, maxElements - currentIndex), currentIndex, element);
            }
          }
        ...
        wrapped.materializeNextWhile(new CancellableIndexedFuturePredicate<E>() {
          ...
          public boolean cancellableTest(final int size, final int index, final E element)
            ...
            if (currentIndex >= maxElements) {
              setDone(EmptyIteratorFutureMaterializer.<E>instance());
              predicate.complete(0);
            } else if (predicate.test(safeSize(size, maxElements - currentIndex), currentIndex,
                element)) {
              if (ImmaterialState.this.index >= maxElements) {
                setDone(EmptyIteratorFutureMaterializer.<E>instance());
                predicate.complete(0);
    ...
    public void materializeSkip(@Positive final int count,
      ...
      if (index >= maxElements) {
        ...
        final int remaining = maxElements - index;
        final int toSkip = Math.min(remaining, count);
    ...
    public int weightHasNext() {
      return index >= maxElements ? 1 : wrapped.weightHasNext();
    }

    @Override
    public int weightNext() {
      return index >= maxElements ? 1 : wrapped.weightNext();
    }

    @Override
    public int weightNextWhile() {
      return index >= maxElements ? 1 : wrapped.weightNextWhile();
    }

    @Override
    public int weightSkip() {
      return index >= maxElements ? 1 : wrapped.weightSkip();
    }
