...
public class RemoveAfterIteratorFutureMaterializer<E> extends
    AbstractIteratorFutureMaterializer<E> {

  private static final Logger LOGGER = Logger.getLogger(
      RemoveAfterIteratorFutureMaterializer.class.getName());

  private final boolean isMaterializedAtOnce;
  private final int knownSize;

  public RemoveAfterIteratorFutureMaterializer(@NotNull final IteratorFutureMaterializer<E> wrapped,
  ...
  private class ImmaterialState implements IteratorFutureMaterializer<E> {
          ...
          public void cancellableAccept(final List<E> elements) throws Exception {
            if (index > numElements) {
              ...
              if (offset < elements.size()) {
                materialized = removeFunction.apply(elements, offset);
              } else {
    ...
    public void materializeHasNext(@NotNull final FutureConsumer<Boolean> consumer) {
      if (index == numElements && !skipped) {
        skipped = true;
        wrapped.materializeSkip(1, new CancellableFutureConsumer<Integer>() {
          @Override
    ...
    public void materializeNextWhile(@NotNull final IndexedFuturePredicate<E> predicate) {
      wrapped.materializeNextWhile(new CancellableIndexedFuturePredicate<E>() {
        ...
        public boolean cancellableTest(final int size, final int index, final E element)
            throws Exception {
          if (ImmaterialState.this.index == numElements && !skipped) {
            skipped = true;
    ...
    public void materializeSkip(@Positive final int count,
      ...
      if (Math.min(Integer.MAX_VALUE, (long) index + count) >= numElements && !skipped) {
        skipped = true;
        wrapped.materializeSkip((int) Math.min(Integer.MAX_VALUE, (long) count + 1),
            new CancellableFutureConsumer<Integer>() {
              @Override
              public void cancellableAccept(Integer skipped) throws Exception {
                if (Math.min(Integer.MAX_VALUE, (long) index + skipped) > numElements) {
