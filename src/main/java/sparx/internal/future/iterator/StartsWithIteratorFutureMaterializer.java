/*
 * Copyright 2024 Davide Maestroni
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package sparx.internal.future.iterator;

import static sparx.internal.future.FutureConsumers.safeConsume;

import java.util.Collections;
import java.util.concurrent.CancellationException;
import java.util.concurrent.atomic.AtomicReference;
import java.util.logging.Logger;
import org.jetbrains.annotations.NotNull;
import sparx.concurrent.ExecutionContext;
import sparx.internal.future.FutureConsumer;
import sparx.util.annotation.Positive;

public class StartsWithIteratorFutureMaterializer<E> extends
    AbstractIteratorFutureMaterializer<Boolean> {

  private static final Logger LOGGER = Logger.getLogger(
      StartsWithIteratorFutureMaterializer.class.getName());

  public StartsWithIteratorFutureMaterializer(@NotNull final IteratorFutureMaterializer<E> wrapped,
      @NotNull final IteratorFutureMaterializer<Object> elementsMaterializer,
      @NotNull final ExecutionContext context,
      @NotNull final AtomicReference<CancellationException> cancelException) {
    super(context);
    setState(new ImmaterialState(wrapped, elementsMaterializer, cancelException));
  }

  @Override
  public int knownSize() {
    return 1;
  }

  private class ImmaterialState extends ImmediateIteratorFutureMaterializerState<E, Boolean> {

    private final AtomicReference<CancellationException> cancelException;
    private final IteratorFutureMaterializer<Object> elementsMaterializer;
    private final IteratorFutureMaterializer<E> wrapped;

    public ImmaterialState(@NotNull final IteratorFutureMaterializer<E> wrapped,
        @NotNull final IteratorFutureMaterializer<Object> elementsMaterializer,
        @NotNull final AtomicReference<CancellationException> cancelException) {
      super(StartsWithIteratorFutureMaterializer.this, wrapped, LOGGER);
      this.wrapped = wrapped;
      this.elementsMaterializer = elementsMaterializer;
      this.cancelException = cancelException;
    }

    @Override
    public int knownSize() {
      return 1;
    }

    @Override
    public void materializeSkip(@Positive final int count,
        @NotNull final FutureConsumer<Integer> consumer) {
      if (!isMaterializing()) {
        setDone(EmptyIteratorFutureMaterializer.<Boolean>instance());
        consumeElements(Collections.<Boolean>emptyList());
        safeConsume(consumer, 1, LOGGER);
      } else {
        super.materializeSkip(count, consumer);
      }
    }

    @Override
    public int weightElements() {
      return isMaterializing() ? 1 : wrapped.weightNext();
    }

    @Override
    public int weightSkip() {
      return isMaterializing() ? weightElements() : 1;
    }

    @Override
    void materialize() {
      new MaterializingConsumer().schedule();
    }

    private @NotNull String getTaskID() {
      final String taskID = context.currentTaskID();
      return taskID != null ? taskID : "";
    }

    private class MaterializingConsumer extends CancellableIndexedFutureConsumer<Object> {

      private Object element;
      private boolean isWrapped = true;
      private String taskID;

      @Override
      public void cancellableAccept(final int size, final int index, final Object element) {
        if (isWrapped) {
          this.element = element;
          isWrapped = false;
          schedule();
        } else if (this.element == element || (element != null && element.equals(this.element))) {
          this.element = null;
          isWrapped = true;
          schedule();
        } else {
          setDone(new ElementToIteratorFutureMaterializer<Boolean>(false));
          consumeElements(Collections.singletonList(false));
        }
      }

      @Override
      public void cancellableComplete(final int size) {
        if (isWrapped) {
          elementsMaterializer.materializeHasNext(new CancellableFutureConsumer<Boolean>() {
            @Override
            public void cancellableAccept(final Boolean hasNext) {
              final boolean result = !hasNext;
              setDone(new ElementToIteratorFutureMaterializer<Boolean>(result));
              consumeElements(Collections.singletonList(result));
            }

            @Override
            public void error(@NotNull final Exception error) {
              setError(error);
            }
          });
        } else {
          setDone(new ElementToIteratorFutureMaterializer<Boolean>(true));
          consumeElements(Collections.singletonList(true));
        }
      }

      @Override
      public void error(@NotNull final Exception error) {
        setError(error);
      }

      @Override
      public @NotNull String taskID() {
        return taskID;
      }

      @Override
      public int weight() {
        return isWrapped ? wrapped.weightNext() : elementsMaterializer.weightNext();
      }

      @Override
      @SuppressWarnings("unchecked")
      protected void runWithContext() {
        if (isWrapped) {
          ((IteratorFutureMaterializer<Object>) wrapped).materializeNext(this);
        } else {
          elementsMaterializer.materializeNext(this);
        }
      }

      private void schedule() {
        taskID = getTaskID();
        context.scheduleAfter(this);
      }

      private void setError(@NotNull final Exception error) {
        final CancellationException exception = cancelException.get();
        if (exception != null) {
          setCancelled(exception);
          consumeError(exception);
        } else {
          setFailed(error);
          consumeError(error);
        }
      }
    }
  }
}
