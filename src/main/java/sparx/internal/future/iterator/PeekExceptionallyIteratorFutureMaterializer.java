...
public class PeekExceptionallyIteratorFutureMaterializer<E> extends
    AbstractIteratorFutureMaterializer<E> {

  private static final Logger LOGGER = Logger.getLogger(
      PeekExceptionallyIteratorFutureMaterializer.class.getName());

  private final boolean isMaterializedAtOnce;
  private final int knownSize;

  public PeekExceptionallyIteratorFutureMaterializer(
      ...
      if (elementsConsumers.size() == 1) {
        wrapped.materializeElements(new CancellableFutureConsumer<List<E>>() {
          ...
          public void error(@NotNull final Exception error) throws Exception {
            try {
              ImmaterialState.this.consumer.accept(wrappedIndex, error);
            } finally {
              setError(error);
            }
          }
    ...
    public void materializeNext(@NotNull final IndexedFutureConsumer<E> consumer) {
      wrapped.materializeNext(new CancellableIndexedFutureConsumer<E>() {
        ...
        public void cancellableAccept(final int size, final int index, final E element)
            throws Exception {
          consumer.accept(size, wrappedIndex++, element);
        }
    ...
    public void materializeNextWhile(@NotNull final IndexedFuturePredicate<E> predicate) {
      wrapped.materializeNextWhile(new CancellableIndexedFuturePredicate<E>() {
        ...
        public boolean cancellableTest(final int size, final int index, final E element)
            throws Exception {
          return predicate.test(size, wrappedIndex++, element);
        }

        @Override
        public void error(@NotNull final Exception error) throws Exception {
          try {
            consumer.accept(wrappedIndex, error);
          } finally {
