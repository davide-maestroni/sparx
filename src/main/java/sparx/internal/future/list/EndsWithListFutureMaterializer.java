...
public class EndsWithListFutureMaterializer<E> extends AbstractListFutureMaterializer<Boolean> {

  private static final Logger LOGGER = Logger.getLogger(
      EndsWithListFutureMaterializer.class.getName());

  public EndsWithListFutureMaterializer(@NotNull final ListFutureMaterializer<E> wrapped,
  ...
  private class ImmaterialState implements ListFutureMaterializer<Boolean> {
                ...
                public void cancellableAccept(final Integer wrappedSize) {
                  if (wrappedSize == 0) {
                    ...
                    elementsMaterializer.materializeElement(elementsSize - 1,
                        new MaterializingFutureConsumer(wrappedSize));
                  }
                }
    ...
    private void setState(final boolean endsWith) {
      consumeState(setDone(new ElementToListFutureMaterializer<Boolean>(endsWith)));
    }

    private class MaterializingFutureConsumer extends
        CancellableIndexedFutureConsumer<Object> implements Task {
      ...
      private MaterializingFutureConsumer(final int wrappedSize) {
